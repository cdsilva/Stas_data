...
%
In this experiment, embryos were immobilized for imaging so that the position and orientation remained fixed \cite{kane1996zebrafish}.
%
...
Although the theory of vector diffusion maps readily extends to three-dimensional data with full three-dimensional rotations, calculation of the pairwise alignments rapidly becomes computationally prohibitive for such applications, as one must now search over all three-dimensional rotations (which are parameterized by three Euler angles), rather than over two-dimensional rotations (which are parameterized by a {\em single} rotation angle).
%
However, ordering data via diffusion maps is readily possible for pre-registered three-dimensional data.
%
