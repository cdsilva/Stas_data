...
%
We use {\it local} information about the data to find a parametrization which respects the underlying manifold geometry, so that points which are close in high-dimensional space (e.g., images which look similar) are close in our parameterization.
%
...
\end{minipage}
\caption{Method validation using live imaging of a zebrafish embryo. (A) Selected images from a movie of zebrafish epiboly. Each frame is in an arbitrary rotational orientation, and the order of the frames has been shuffled. (B) Images from A after registration and ordering using vector diffusion maps. The real time of each frame is also indicated. (C) Correlation between the rank recovered using vector diffusion maps and the true rank.  The rank correlation coefficient is $0.9954$. The larger errors in the recovered ranks towards the beginning of the trajectory are due to the slow cell movement within that time window. }
\label{fig:zebrafish}
\end{figure*}
...
%image registration and ordering can correctly recover image orientation and  showed that our algorithm is not limited to a specific set of data.
%
...
%
%However, with advances in imaging techniques, taking multiple z-slices to visualize the three-dimensional structure of a sample will become more common, and it is important to validate that our algorithm can be applied to such data.
...
%
Therefore, we can apply our methodology to three-dimensional data, such as a stack of confocal images taken at different depths within the sample, by first registering the data using an alternative method, and then ordering the registered data using diffusion maps.
...
For this particular system, the face of the imaginal disc is significantly larger than the edge of the disc, so that images will always be taken with the face towards the microscope.
%
...
%
We aligned the maximum intensity projections using angular synchronization, and then used these rotations to register the full three-dimensional in the x-y plane.
%
...
%
Even with such obstructions, we can order the images with high accuracy and extract a stereotypical developmental trajectory, shown in \fig~\ref{fig:wing_disc}C, by averaging (see \SI).
%
...
%The vector diffusion maps algorithm has been previously used for a different type of three-dimensional data: to reconstruct a three-dimensional object (in this case, a molecule) from two-dimensional (cryo-electron microscopy) pictures \citep{singer2011viewing}.
%
...
%
Temporal ordering of large-scale data was done in the context of molecular profiling studies, in which data are vectors describing the expression levels of different mRNA \citep{anavy2014blind, trapnell2014dynamics, gupta2008extracting}.
%
...
%
%The methodology we have presented here only addresses images taken in the same location and/or from the same viewing direction within the organism.
%
...
%
%Furthermore, as we have acknowledged, our methods as implemented cannot register three-dimensional data with respect to three-dimensional rotations due to computational limitations.
%
%In the future, the algorithms presented here could be used for three-dimensional voxel data given an alternative method for computing pairwise alignments more efficiently (such as using image features or landmarks).
%
