...
%
A schematic illustration of angular synchronization is shown in \fig~\ref{fig:schematics}A, where each image is represented as a vector, and the goal is to align the set of vectors.
%
...
\fig~\ref{fig:drosophila_live_imaging}A shows selected images from this live imaging data set, which contains $40$ consecutive frames taken at $30$~second time intervals at a fixed position within a single embryo (see \fig~\ref{fig:full_movie_data}).
%
...
%
Quantitatively, the rank correlation coefficient for this data set is $0.9936$, and the average error in the recovered angle is $4.23^\circ$. 
...
%
\fig~\ref{fig:drosophila_fixed_images}A shows selected images from a set of $120$ images (see \fig~\ref{fig:full_fixed_data}) which cover a thirty minute time interval spanning late cellularization through gastrulation.
%
...
Up to this point, we have focused on two-dimensional imaging studies. 
%
However, it is not uncommon to encounter three-dimensional imaging studies. 
%
To illustrate how our methods can be extended to three-dimensional data, we analyzed a wing disc data set.


Although the theory of vector diffusion maps readily extends to three-dimensional data, calculation of the pairwise alignments rapidly becomes computational prohibitive for three dimensional data, as one must now search over three Euler angles, rather than a single rotation angle. 
%
However,  ordering data via diffusion maps is readily possible for three-dimensional data. 
%
Therefore, we can apply our methodology to three-dimensional data by first registering the data using an alternative methodology (such as registering max projections, or aligning a single feature of the data), and then ordering the registered data using diffusion maps. 

For the wing disc data, we first registered the three-dimensional data by computing the optimal rotations to align the maximum projections of the data using vector diffusion maps.
%
Because the maximum projections are two-dimensional images, this step is no more computationally intensive than the previous examples. 
%
We then used the computed alignments to register the full three-dimensional data, and then computed the diffusion maps embedding of the three-dimensional registered data. 
%
The results of ordering the data using the first (non-trivial) diffusion maps coordinate are shown in \fig~\ref{fig:wing_disc}. 

The data was sorted into $6$ time cohorts by an expert; the time cohort for each image in \fig~\ref{fig:wing_disc}B are indicated. 
%
The rank correlation coefficient between the ordering from diffusion maps and the expert cohort sorting is 0.9427, indicating that our ordering is consistent with the expert. 
...
Wing disc & 3D & 3 & 46 & 100 $\times$ 100 $\times$ 21 & 10$^\circ$ & 12~sec  \\
 \hline
\end{tabular}}}}
\caption{Computational requirements for the presented methodology. (A) CPU time as a function of the number of images in the data set (for $100 \times 100$ pixel images, and $10^\circ$ rotational discretizations). (B) CPU time as a function of the number of pixels in the images (for $120$ images, and $10^\circ$ rotational discretizations). (C) CPU time as a function of the number of rotations (for $120$ images of $100 \times 100$ pixels). (D) The algorithm settings and computational requirements for the data sets analyzed. All times are reported for an Intel Core i7 2.93 GHz processor.}
\label{fig:cpu_time}
\end{figure*}
...
%
The computational costs for an Intel Core i7 2.93 GHz processor are outlined in \fig~\ref{fig:cpu_time}.
%
...
%
Furthermore, the computation of the pairwise rotational alignments is trivially parallelizable for larger data sets.
%
We subsampled all of our data sets to $100 \times 100$ pixels, as we found that this resolution still captured all of the relevant developmental features. 
...
$\mathcal{O}(n^{1.83})$ in number of pixels
$\mathcal{O}(n^{1.33})$ in number of images
$\mathcal{O}(n^{0.77})$ in number of rotations
