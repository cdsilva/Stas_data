function im2 = warp_image_rbf(im1, cp1, cp2, sigma)
...
% calculate differences in control points
cp_deltas = cp2 - cp1;
...
if ndims(im1) == 2
    im2 = uint8(griddata(X+X_deltas,Y+Y_deltas,double(im1),X,Y));
else
    im2 = zeros(size(im1), 'uint8');
    for i=1:size(im1, 3)
        im2(:,:,i) = uint8(griddata(X+X_deltas,Y+Y_deltas,double(im1(:,:,i)),X,Y));
    end
end
