...
% movie
[images_movie, time_movie] = load_contour(npixels);
nimages_movie = length(time_movie);
time_movie = time_movie + 40; 
...
for i=1:nimages_movie
    ...
    images_movie_colored(:,:,:,i) = imlincomb(1, images_movie_colored(:,:,:,i), 0.25, repmat(images_movie(:,:, i), 1, 1, nchannels));
     images_movie_colored(:,:,:,i) = immultiply(images_movie_colored(:,:,:,i), repmat(im2bw(images_movie(:,:, i)), 1, 1, nchannels));
end

%%

writerObj = VideoWriter('colored_movie_countour.avi');
