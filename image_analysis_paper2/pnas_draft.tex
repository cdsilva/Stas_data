...
\caption{{\xfigtextfontit Drosophila} embryos during cellularization. {\xfigtextfontit (A)} Selected images from a live movie of a {\xfigtextfontit Drosophilla} embryo during gastrulation. Each frame is in an arbitrary rotational orientation, and the order of the frames has been shuffled. {\xfigtextfontit (B)} Images from {\xfigtextfontit A} after registration and ordering using vector diffusion maps. The dorsal side of each embryo now appears at the top of each image, and the ventral side appears at the bottom. {\xfigtextfontit (C)} The correlation between the recovered rotation angle (using vector diffusion maps) and the true rotation angle for each frame in the movie. {\xfigtextfontit (D)} The correlation between the recovered rank (using vector diffusion maps) and the true rank for each frame in the movie. {\xfigtextfontit (E)} The average error in the recovered angle and the rank correlation coefficient for $5$ different movies. {\xfigtextfontit (F)} The average rank correlation coefficient as a function of the number of images in the data set. Each point is averaged over $200$ samples. }
...
%
This data set was obtained through live imaging of a single {\it Drosophila} embryo during the twenty minutes spanning late cellularization through gastrulation.
%
...
%
However, arguably the most common source of noise within developmental biology experiments is interembryo variability. 
%
To measure the sensitivity of our proposed methodology to interembryo variability, we constructed synthetic time course data sets by selecting a frame from a random movie for each time point.
% 
When using vector diffusion maps to order these data sets, the median rank correlation coefficient was found to be 0.81, indicating that our methodology can still accurately recover the temporal order even under noisy conditions.  
...
%
We also showed that these results are consistent across multiple data sets, demonstrated that the method also performs well for data with interembryo variability, and showed how the accuracy of our methods varies as a function of the number of images.


\begin{figure*}[t]
\includegraphics{fig4}
\caption{{\xfigtextfontit Drosophila} embryos during gastrulation. {\xfigtextfontit (A)} Images of gastrulating embryos. Each image is of a different embryo arrested at a different developmental time and in a different rotational orientation. {\xfigtextfontit (B)} Data from {\xfigtextfontit A}, registered and ordered using vector diffusion maps. The images are ordered from left to right and from top to bottom, so that the first image appears in the top left position of the array, and the last image appears in the bottom right. {\xfigtextfontit (C)} A representative ``developmental path'' from local averaging of the images. }
...
Live imaging data, such as the images presented in the previous section, represent only a small fraction of the imaging data collected in developmental biology experiments. 
%
Often, data is collected from many multiple embryos, each of which is fixed to arrest its development and contributes only a snapshot of a chemical or morphological process along its developmental trajectory \cite{jaeger2004dynamic, peter2011gene, fowlkes2008quantitative}.
%
...
a certain time window to which a collection of embryos belongs \cite{ng2012large, richardson2014emage, castro2009automatic}.
%
In order to recover the developmental dynamics from such data sets, snapshots of different embryos must be registered and then ordered in time.

\fig~\ref{subfig:fixed_images_unregistered_unordered} shows selected images from a set of $126$ images (see \SI  for the full data set) which cover a thirty minute time interval spanning late cellularization through gastrulation.
%
...
%
Another signal is provided by the phosphorylated form of the extracellular signal regulated kinase (dpERK, shown in red), an enzyme that specifies a subset of neuronal cells.
...
Because the developmental time of each embryo cannot be easily estimated, we have few options for validating our ordering. 
%
We compared the ordering obtained from vector diffusion maps to a manual ordering provided by a trained embryologist who is aware of the developmental progression and the important image features. 
%
The results, shown in  \fig~\ref{subfig:fixed_images_rank_corr}, show that our ordering is consistent with the manual ordering.
%
...
%
Empirically, we found the computational time to be polynomial in the number of images, number of pixels, and the number of discretization points used to compute the pairwise rotations. 
%
...
%
Images must first be preprocessed so that the Euclidean distance between the pixels in informative; for our examples, this amounts to cropping the images to normalize the sizes of the embryo cross-sections, as well as blurring and normalizing the intensity of the nuclear signal, as the absolute intensity of this signal is not meaningful. 
%
...
%
We found that the results are not particularly sensitive to either of these parameters (5$^{\circ}$--30$^{\circ}$ angle discretizations and kernel scales of one-fifth to one-twentieth of the median pairwise distances all yielded acceptable results)
...
%
Although we have presented results for a specific type of data (cross-sectional images of developing {\em Drosophila} embryos), because the methodologies only rely on computing distances between images, we are confident that they can be readily applied and extnded to many other imaging data sets. 
...
%
In general, the size of the data set required is a function of the instrument noise, interembryo variablity, and the length and complexity of the developmental dynamics.
%
