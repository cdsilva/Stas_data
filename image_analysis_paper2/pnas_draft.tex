...
%
To illustrate this approach, we temporally order and register data sets collected
in studies of cell signaling in the early fruit fly embryo.}
...
%
In one of the most commonly used experimental formats, developmental progress is reconstructed from snapshots of chemical or mechanical processes in fixed embryos.
%
As a first step in these reconstructions, such snapshots must be spatially registered and ordered in time.
%
Currently, registration and ordering of images is often done manually, requiring a significant amount of expertise with a specific developmental system.
%
...
%
We validate our approach using a live imaging data set where the correct rotational orientation and temporal order are known.
%
Finally, we demonstrate that our approach can successfully extract a representative developmental trajectory from a collection of images of fixed {\it Drosophila} embryos.
...
%
Often in practice, each image comes from a different embryo, and the largest source of noise in the considered data set arises from embryo-to-embryo variability.
%
...
%
This data set is more complex than the live imaging data sets in that it contains significantly more images, each of which provide information about tissue morphology and the spatial distribution of two regulatory proteins.
%
...
%
In contrast, our method requires relatively little computational time; registering and ordering this fixed imaging data set took $15$~seconds on an Intel Core i7 2.93 GHz processor. %for $100$ images of $100 \times 100$ pixels, using 10$^{\circ}$ rotational discretizations.
%
...
%
The tasks of image preprocessing and parameter selection are relatively simple compared to manual registration and ordering of images, and so this methodology is promising for much larger imaging data sets which are impractical to evaluate manually. 
...
%
Temporal ordering of large-scale data which did not require registration was done in the context of molecular profiling studies, in which data are vectors describing the expression levels of different mRNA.
%
...
such methods should be sufficiently general to address a wide variety of applications
%
