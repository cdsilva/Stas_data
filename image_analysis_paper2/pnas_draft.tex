...
%
We show that the algorithm reproducible recovers the correct orientation and temporal order in data sets where they are independently known, and can reconstruct an average developmental trajectory when such information is unavailable.
...
As a first illustration, we analyzed a data set where the correct registration and temporal order is known {\it a priori}.
%
This data set is obtained by live imaging of a single {\it Drosophila} embryo during the twenty minute time interval spanning late cellularization through gastrulation.
%
...
The images show an optical cross-section of a vertically oriented developing embryo, where the nuclei are labeled by DAPI, a DNA stain.
%
The data set contains $42$ frames, taken at $30$~sec time intervals;
\fig~\ref{subfig:movie_unregistered_unordered} shows selected images from this data set. 
%
The gray nuclear signal was blurred to obtain a smoother images.
%
Each frame was been arbitrarily rotated, and the order of the frames was scrambled.

We used vector diffusion maps to register and order the images. 
%
\fig~\ref{subfig:movie_registered_ordered} shows the same selected images, now registered and ordered, and the real time for each frame is indicated.
%
Note that the recovered ordering is consistent with the real time for each frame. 
%
...
%
The errors in the recovered angles are all very low, and the rank correlation coefficients are consistently close to 1, indicating that our methodology can reproducibly order data of this type.

We used bootstrap sampling to determine how the accuracy of the recovered orderings varies as a function of the size of the data set.
%
\fig~\ref{subfig:movie_bootstrap} shows the average rank correlation coefficient as a function of the number of images.
%
The images were sampled (with replacement) from the original data set of $42$ images, and each point is the average over $200$ bootstrap samples.
%
...
We can now demonstrate the utility of our methodologies on a data set where the correct ordering is not known {\em a priori}.
%
...
\fig~\ref{subfig:fixed_images_registered_ordered} shows the images registered and ordered using vector diffusion maps \cite{singer2012vector}.
%
...
%
To highlight the developmental dynamics revealed by vector diffusion maps, we divided the ordered trajectory into $14$ groups, and computed the average image in each group.
%
...
%
The dorsal dpERK peak then dissapears during germband extension as cells from the ventral side wrap around to the dorsal side.
%
...
%
The CPU time required scaled linearly with the number of pixels and the number of rotations, and scaled to the 1.5 power with the number of images in the data set.
%
...
%
We therefore require automated methodologies which are sufficiently general to address a wide variety of applications to organize such large data sets.
%
...
%
In the examples presented here, we have focused on recovering a single coordinate which is used to effectively order the images in time.
%
...
%
Images taken from different viewing directions can be analyzed in this way, as the vector diffusion maps parameterization will organize the images according to the viewing angle.
%
