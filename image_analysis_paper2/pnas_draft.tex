...
%
Often data is collected in several experiments across many samples, and the first task is to organize the data for further analysis
%
...
%
We validate this approach using a live imaging data set where both the correct rotational orientation and temporal order are known, as well as demonstrate using these methods to extract a developmental trajectory from a data set of fixed images to study dorsoventral patterning of the {\it Drosophila} embryo.
...
%The way it goes now is that its not only not known a priori, but that we have embryo to embryo variability, both in sizes of embryos and in the levels of signals extracted from images. To test our algorithm under these more challenges conditions, we collected a dataset where......
