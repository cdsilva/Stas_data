function [ class_VDM, angle, VV_LP ] = VDM_noReflection(class, corr, rot, k, flag, n_nbor) 
...
[ X, Ah, rows, cols ] = sort_list_weights_wrefl(class(:, 1:k), sqrt(2-2*real(corr(:, 1:k))), rot(:, 1:k), ones(P, k) );
if flag==0
...
[r_VDM_LP, ~, VV_LP]=VDM_LP(H2, 3);

if P<=10^4
    corr_VDM=r_VDM_LP(1:P, :)*r_VDM_LP';
    corr_VDM=real(corr_VDM-sparse(1:P, 1:P, ones(P, 1), P, P));
...
list=[repmat([1:P]', n_nbor, 1), class_VDM(:)];
%[ angle ] = VDM_angle_v2(VV_LP(:, 1:10), list);
[ angle ] = VDM_angle_v2(VV_LP, list);
