...
\dropcap{E}xperimental studies of developmental dynamics fall in two broadly defined categories: longitudinal \cite{...} and cross-sectional \cite{...}.
%
...
%
In order to mine such image datasets for underlying developmental dynamics, snapshots of different embryos must be spatially aligned ("registered")  and ordered in time.
%
...
%
Instead, we (approximately) represent rotations and translations in two-dimensions using three-dimensional rotation matrices, by projecting the image onto a portion of the surface of a (three-dimensional) sphere \cite{singer2011angular} (see {\it SI Appendix}).
%
...
%
Practically, the images are discretized at pixels (rather than continuous functions on the unit sphere).
%
We therefore discretize the optimization problem, and for each image pair $I_i$ and $I_j$, we solve
...
%
We then convert the two-dimensional translation and rotation to a (continuous) three-dimensional rotation.
%
...
%
From the matrices $R_{1, est}, \dots, R_{m, est}$, we can then recover the corresponding translations and rotations of the images.
%
