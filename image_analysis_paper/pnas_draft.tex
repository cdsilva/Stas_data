...
%
If each data point is a two-dimensional image, the images must first be registered (aligned) before they can be temporally ordered.
%
...
%
We present an automatic approach to registering {\em and} temporally order cross--sectional data sets of images.
%
...
%
In a cross-sectional study, one embryo contributes only a single snapshot of a chemical or morphological process along its developmental trajectory; the latter must be reconstructed from multiple snapshots of different embryos.
%
...
%
In order to mine such image datasets for underlying developmental dynamics, snapshots of different embryos must be spatially aligned ("registered") to factor out the relevant symmetries (translations and rotations),  and ordered in time.
%
We present a general computational framework that greatly accelerates -and can combine- both of these tasks.
...
%
In this case, temporal ordering can be accomplished by arranging the fish by size, which uniquely correlates with the progress of development; image registration is then based on obvious morphological landmarks, such as positions of head and fins.
%
Real data will, of course, provide nontrivial challenges, due to the number of images, due to variability and measurement noise, and due to the subtlety of several developmental changes.

%
To illustrate the issues associated with temporal ordering and registration of images, we use a data set from an imaging study of regulatory protein expression
%%%
%%% check whether "regulatory protein" is the correct term
%%%
pattern formation in the early fruit fly embryo, one of the leading model systems for studies of developmental dynamics.
%
...
%
Each of the images reveals a uniform arrangement of nuclei and an azimuthally nonuniform distribution of the two proteins, which are involved in patterning of the dorsoventral (back-to-belly) axis of the embryo.
...
%
The goal is to register these images in a globally consistent way and order them in a way that is most representative of the underlying developmental trajectory.
%
...
%
We discretize the profiles into $100$ points, so our numerical data will be $100$-dimensional vectors.
%
Rotation of the ring in Figure~\ref{subfig:1d_example} corresponds to shifting (with periodic boundary conditions) the one-dimensional concentration profile shown below the ring (the continuous symmetry group is the group of all two-dimensional proper rotations, denoted $SO(2)$; the corresponding discrete symmetry group for the discretized profiles is $\mathbb{Z}_n$, the group of integers modulo $n$).
%
Figure \ref{subfig:1d_unaligned_unordered} shows all the concentration profiles in our data set; they have been stacked in an array, so that each row corresponds to a single profile.
%
...
\caption{Illustration of methodologies using one-dimensional profiles. (a) One-dimensional concentration profile on a ring (top), and the corresponding profile on a line (bottom). (b) Intensity corresponding to the profile in (a). (c) An ensemble of concentration profiles, each of the form described in (a). Each row in the array corresponds to a single image. (d) The concentration profiles in (c), now registered using angular synchronization. (e) The concentration profiles in (c), now temporally ordered using diffusion maps. (f) The concentration profiles in (c), registered and temporally ordered in a single step using vector diffusion maps.}
%\label{fig:1d_demo}
...
We start by registering the set of images or profiles.
%
...
%
Angular synchronization first aligns every possible pair of data points; that is, it computes the rotation angle
required to make them have the maximum possible overlap (see {\it Materials and Methods}).
%%%
...
%
From this large collection of pairwise alignments, angular synchronization then selects a unique (up to a global rotation)
set of angles $\theta_i$, one for each data point, such that the resulting pairwise alignments $\theta_i-\theta_j$ are {\em most consistent} in some metric.
%
Figure~\ref{subfig:synch2} shows each vector from Figure~\ref{subfig:synch1}, now rotated so that the entire set is aligned
(the rotation angles we computed are also indicated).
%
...
provides the (optimal) rotation angles assigned to each data point.
%
...
\caption{(a) Set of vectors, each in a different orientation. The angles to align one vector to another are indicated. (b) The vectors from (a), each rotated so that the set is globally aligned. Note that the chosen rotation angles are consistent with the pairwise alignments in (a). (c) Data points (in black) which lie on a one-dimensional nonlinear curve in two dimensions. Each pair of data points is connected by an edge, and the edge weight is related to the Euclidean distance between the points through the diffusion kernel (see \eqref{eq:dmaps_W}), so that close data points are connected by dark (``stronger'') edges. (d) The data in (c), colored by the first (non-trivial) eigenvector from the diffusion map computational procedure, $\phi_2$. The color intensity is monotonic with the arclength, thus parameterizing the curve.  }
...
%
It is reassuring that the intensity peaks appear aligned,
...
After geometrically registering the data (images or concentration profiles), we want to order them in time to reconstruct the developmental dynamics.
%
Our main assumption is that data variability
strongly correlates with time (see below).
%
We use diffusion maps to temporally order our data.
%
In general, diffusion maps is a dimensionality reduction technique, which aims to uncover low-dimensional, nonlinear structure in high-dimensional data sets \cite{coifman2005geometric}.
%
...
%
Assume that our (one dimensional, or two-dimensional image) data also lies along a one-dimensional, nonlinear curve in a high-dimensional space, and that this curve is parameterized by time (e.g., in Figure~\ref{subfig:dmaps_color}, we assume that the data points are ordered from youngest to oldest along the curve).
%
...
%
To emphasize this point, edges between the pairs of data points in Figure~\ref{subfig:dmaps_edges} are colored according to the distance between the data points, so that close data points have dark (``strong", ``meaningful") edges connecting them.
%
Visually, these dark edges delineate the one-dimensional curve; in contrast, the light edges are much less informative
...
%
This problem can be formulated as an optimization problem \cite{Belkin2003}, and the solution is given by the top (non-trivial) eigenvector of a Markov matrix which contains information about pairwise distances between the data points.
%
...
%
Angular synchronization (for registration) can be combined with diffusion maps (for temporal ordering) in a single eigencomputation that allows us to {\em simultaneously} recover the optimal alignments {\em and} the right temporal ordering.
%
...
%
The data has been ordered by $\psi_{1, 3}$, the first (non-trivial) vector diffusion maps coordinate (see {\it Materials and Methods}).
...
two color, two-dimensional images (discretized into $100 \times 100$ pixels, or $20000$-dimensional vectors).
%
We first register the images with respect to translations and rotations (the appropriate continuum symmetry group is $ISO(2)$);
it is expedient to instead approximate the two-dimensional translations and rotations by {\em three-dimensional rotations}, for which the symmetry group $SO(3)$ facilitates the application of the
vector diffusion map algorithm (see both {\it Materials and Methods} and {\it SI Appendix})
%
%%%
%%% AGAIN ?
%%%
...
%
Both the red and green peaks are now aligned, and the intensity of the red peaks grows as a function of time (or of the leading vector diffusion map coordinate).

In these experiments, we are fortunate enough to be able to validate our computations of automatic registration and ordering using prior physical knowledge about the system.
%
For registration, it is known that the peak in the distribution of Dorsal (visualized by the green channel) specifies the ventralmost position in the embryo, and that the nuclei (shown in gray) delineate the circumference of the embryo.
%%%
...
%
The rank correlation coefficient between these two temporal orderings is 0.9149, indicating that our automatic ordering is consistent with the ordering obtained from cellularization.
%

The simultaneous registrations/orderings can be visually compared through the sequences of one-dimensional profiles (similar to the format in Figure~\ref{fig:1d_demo}), shown in Figures~\ref{subfig:1d_membranes}~and~\ref{subfig:1d_vdm}.
%
...
%
They have been shown to be more robust to noise than other path-based algorithms, such as isomap \cite{balasubramanian2002isomap}; we therefore suspect they may perform better than the ordering algorithms used in previous work.
%%%
%%%   we need help here
%%%
%
...
%
Temporal ordering of images has been studied, for example, in ordering images taken of a construction site over time \cite{schindler2007inferring}; these methods are also based on first extracting features from the images.
%
However, in general, appropriate features may not be known {\em a priori}, and so we require a registration algorithm that does not rely on predefined features, but only on the inherent geometric symmetry of the problem.
%
Template methods are also common for registration, where one optimally aligns each image to a predefined template function.
%
...
%
Angular synchronization and vector diffusion maps have been used to register two-dimensional cryo-electron microscopy images of three-dimensional molecules \cite{singer2011three}; there each image is a picture of (essentially) the {\em same} molecule, and variation in the data comes from differences in the viewing angles (rotations of the molecule) and from instrument/measurement noise.
%
...
%
These synchronization methods are formulated for general symmetry groups.
%
Depending on the specific data, one might consider other symmetries, such as reflections and scaling/dilation.
%
For synchronization, our symmetry group must be such that...

Temporal ordering can also be accomplished without first registering the images, by constructing {\em features} which are invariant to the underlying symmetry group. 
%
In such methods, one computes functions of the images which are invariant under translations and rotations. 
%
...
%
However, such methods can often be faster for large data sets, since sets of features are computed per image, whereas synchronization requires pairwise computations.
...
%
In addition, the data should contain spatial variability for synchronization to successfully register the images.
%
...
%
Visually, one can see that the green channel does not change appreciably in time; if we only used this channel in our analysis, the principal direction of variability would not be correlated with time, and we could not temporally order the data using diffusion maps.
%
...
%
This variability allows us to register the data points.
%
...
Given $m$ data points $x_1, \dots, x_m$ (typically vectors in a high-dimensional space), we want to find a coordinate transformation (in our case, we are interested in finding a single coordinate) $y(x)$ that preserves local information: points that are close in the original space should also be close in the coordinate $y$.
%
...
We want a coordinate $y$ such that $y(x_i)$ and $y(x_j)$ are close if $W_{ij}$ is large.
%
This is accomplished solving following optimization problem \cite{Belkin2003}
...
%
The next eigenvector, $\phi_2$, is the (non-trivial) solution to \eqref{eq:dmaps_opt_problem}, so that $\phi_2(j)=y(x_j)$ gives the ``new" coordinate for data point $x_j$.
%
...
\end{equation}
%
where $A_{ij} \in \mathbb{R}$ pertains to the diffusion kernel for the pair of data points $x_i, x_j$ and is defined in \eqref{eq:dmaps_A}, and $H_{ij} \in \mathbb{R}^{d \times d}$ pertains to the pairwise alignment between the data points and is defined in \eqref{eq:H_to_R}.
%
It is important to note that distance $d(x_i, x_j)$ used in the diffusion kernel in \eqref{eq:dmaps_W} is the distance between data points {\em after} after pairwise alignment; that is, the minimum obtained in \eqref{eq:opt_pairwise} among all possible distances between all possible shifts of the two data points.
%
...
to be  $\psi_{1,d+1}$.
%
Our example is, by physical considerations, a separable one; if no separability claims can be made,
it is conceivable that a second diffusion map step, using $\psi_{k,l}$ as the coordinates, might uncover a
...
\end{equation}
where $g(I_j, \theta, dx, dy)$ is a function that results from first rotating the image $I_j$ around the center of the square by $\theta$ degrees, then translating the resulting
...
%
This also explains the $\Delta$ limits in the translation: 
%
...
%
The missing pixels in the rotated image (the ``corner effects" we mentioned above) are taken to be black (zero intensity).
%
...
%
Our ``quantum translation" is chosen to be $1/10^{th}$ of the domain in each of the $x$ and $y$ directions, while
our ``quantum rotation" is chosen to be $1/20^{th}$ of $2 \pi$. 
%
...
%
We then check all possible combinations for the rotation and translations that best align $I_j$ to $I_i$.
%
...
The requirements for the synchronization step (e.g. for \eqref{eq:H_low_rank} to be satisfied), include the condition that the matrices used to represent the symmetry group be orthogonal, i.e., $R(\alpha)^{-1} = R(\alpha)^T$ \cite{singer2013spectral}.
%
However, the typical matrix representation for the group of two-dimensional translations and rotations, $ISO(2)$, does not satisfy this property.
%
...
We thus convert the optimal two-dimensional translation and rotation to corresponding (continuous) three-dimensional rotations.
%
...
These rotation matrices $R(\alpha_{ij}, \beta_{ij}, \gamma_{ij})$ are used in formulating the matrix $H$ in \eqref{eq:H_to_R} for angular synchronization, or the matrix $S$ in \eqref{eq:vdm_S} for vector diffusion maps, to compute the estimates of the $R_{i, est}$ in \eqref{eq:R_est}.
%
From the matrices $R_{1, est}, \dots, R_{m, est}$, we can then recover the corresponding (approximately) optimal translations and rotations of the images.
%
...
\end{equation}
%
Inverting \eqref{eq:angle_relations} yields the corresponding approximate translations and rotation;
optimal translations to the nearest integer to remove the need for interpolation when translating the images.
