...
%
In addition, the task of image registration has been widely studied \cite{zitova2003image}, for applications such as face recognition \cite{rowley1998rotation}, medical image registration \cite{hajnal2010medical}, and texture classification \cite{greenspan1994rotation}.
%
...
%
In the first case, correct orientation and temporal order of images was known {\it a priori}, which allowed us to validate the results of the algorithm.
%
Both the orientation and temporal order were recovered with high accuracy. 
%
However, for this particular dataset, a simpler approach would suffice. 
%
Because of the relative simplicity of developmental dynamics (monotonic increase in the amplitude of a localized signaling pattern), temporal order could have been recovered by projection of registered images on the first principal component. 
%
However, PCA would clearly not suffice for the second dataset, collected from embryos during the time window where both the tissue morphology and signaling patterns are rapidly changing.  
%
...
Angular synchronization and vector diffusion maps have been used to reconstruct molecular shapes from cryo-electron microscopy imaging data \cite{shkolnisky2012viewing, singer2012vector}. 
%
Due to the high levels of noise in these images, thousands of images are needed for successful shape reconstruction. 
%
