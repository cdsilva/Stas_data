...
%
We use a manifold learning algorithm \cite{singer2012vector} which simultaneously addresses the problems of registration and temporal ordering. 
%
...
%
The highly reproducible kinetics of lateral membrane growth can be used to estimate the age of each embryo to within 1--3 minutes \cite{figard2013plasma} (see \fig~S2).
%
...
%
To illustrate this, we collected a data set consisting of $42$ embryos undergoing cellularization during the third hour of development (see \fig~S3).
%
...
We computed the vector diffusion maps embedding for our data, and the eigenvalue spectrum (see \fig~S6) indicates that two coordinates may be required for a thorough description of the data. 
%
...
%
This approach was clearly not sufficient for the second data set (see \fig~S4), collected from embryos during the time window where both the tissue morphology and signaling patterns are rapidly changing.  
%
