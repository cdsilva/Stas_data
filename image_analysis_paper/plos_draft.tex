...
The issue of rotation invariance arises in many applications, most commonly image registration and image alignment.
%
In settings such as face recognition, medical imaging, and computer vision, objects from different frames or timepoints must be aligned or ``registered'' before further analysis can be done.
%
...
%
Most of these techniques are ``template-based'' techniques, where one selects a prototypical image or ``template'', and then aligns the remaining images to this template \cite{...}.
%
...
In addition, most existing techniques rely on feature extraction algorithms, and these features are often problem-specific.
%
For example, (ADD INFO ABOUT FACES).
...
%
We then use image analysis software to extract the circular dpERK concentration profile from the fluorescent image.
%
...
%
We then measure the membrane thickness of them embryo; the thickness of the membrane is known to be monotonic in time during NC14 \cite{...}. 
%
