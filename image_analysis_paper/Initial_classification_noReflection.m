function [ class, rot, corr, FBsPCA_data, timing ] = Initial_classification_noReflection(data, r_max, n_nbor, isrann )
...
[ Coeff_b, ~, toc_bispec ] = Bispec_2Drot( Coeff, Freqs );

if P<=10000
    %%For small dataset, search for nearest neighbors
    tic_nn=tic;
    corr=real(Coeff_b'*Coeff_b);
    corr=corr - eye(P);
    ...
    if ~isrann
        ...
        for i=1:ceil(P/P_max)
            corr=real(Coeff_b(:, (i-1)*P_max+1:min(i*P_max, P))'*Coeff_b);
