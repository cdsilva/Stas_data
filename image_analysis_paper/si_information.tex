...
Retain both the optimal pairwise alignments (translations and rotations) as well as the minimum distances obtained when pairs are optimally aligned.
%
\item Compute the corresponding pairwise three-dimensional rotations from the translations and rotations, using \eqref{eq:angle_relations}, \eqref{eq:three_rot}, and \eqref{eq:total_rot}.
%
\item Select $\epsilon$ for use in the diffusion maps kernel. 
We often begin with using the median of the pairwise distances as $\epsilon$, and then adjust as needed.
\item Compute the matrix $S$ in \eqref{eq:vdm_S}. 
$A_{ij}$ is computed from \eqref{eq:dmaps_W} and \eqref{eq:dmaps_A}, using the pairwise distances from step 1, and $H_{ij} = R(\alpha_{ij}, \beta_{ij}, \gamma_{ij})$, which are computed in step 2.
%
...
\item For each image $i$, compute the optimal translation and rotation to align the image by converting $R_{i, est}^T$ (note the transpose) to the corresponding translation and rotation, described in \eqref{eq:angle_invert}. 
\item Compute the first embedding coordinate $\psi_{k,1}(i)$ as described in \eqref{eq:vdm_coord} and \eqref{eq:first_embed_vdm}, where $4 \le k \le 6$.
...
%
%\begin{figure*}[t]
%{\bf Item C in the figure below is our main observation.}
%
%\raisebox{7.6cm}{\sffamily{A}}
%\includegraphics[width=8cm]{raw_data1}
%\hfill
%\raisebox{7.6cm}{\sffamily{B}}
%\includegraphics[width=8cm]{FBsPCA_evals}
%
...
%%
%However, (mainly, we suspect) because the bispectrum is not stable to deformations, it does not seem to be able to identify neighboring images.
%%
%(A) Images used for FBsPCA. (B) Eigenvalue spectrum from diffusion maps, computed using bispectrum coefficients. There is no visible spectral gap after the first component, as would be expected for this particular data set. (C, top) First 4 diffusion maps variables, computed from the bispectrum coefficients, plotted against the membrane thickness. Note there is no appreciable correlation: {\bf each diffusion maps variable simply selects one image from the data set as ``abnormal''.} (C, bottom) The corresponding images that each diffusion maps mode ``selects.'' In general, these seem to be the late images with ventral furrows beginning to form. }
%\end{figure*}
