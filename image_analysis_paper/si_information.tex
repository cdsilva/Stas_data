...
%
The $j^{th}$ embedding coordinate will be given by $\psi_{k,l}$ such that $0 < k \le d, jd +1 < l \le (j+1)d$,
and we select the $(k,l)$ pair such that the coordinate $\psi_{k, l}$ has the largest variability. 
%
...
%
\item To compute subsequent coordinates, use lower blocks for $l$ (e.g., for the next coordinate, under translations and rotations, use $k=1, 2, 3$ and $l = 7, 8, 9$). Again, compute $\psi_{k, l}$ as in \eqref{eq:vdm_coord}, and select the $(k, l)$ coordinate with largest variance.
