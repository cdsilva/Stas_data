...
W = squareform(pdist(train_coeff(:, PCA_idx)));
eps = median(W(:));

[V_dmaps, D_dmaps] = dmaps(W, eps, 10);

figure;
scatter(train_times, V_dmaps(:,2),200, train_movie_idx, '.')

figure;
scatter(V_dmaps(:,2), V_dmaps(:,3), 200, train_times, '.')
