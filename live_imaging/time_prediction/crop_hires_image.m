function image2 = crop_hires_image(image, channel)
...
npix_filt = round(0.1*npixels);

if ndims(image) == 3
    im_tmp = medfilt2(image(:, :, channel), [npix_filt npix_filt]);
    im_tmp = edge(im_tmp, detection_type);
else
    im_tmp = medfilt2(image, [npix_filt npix_filt]);
    im_tmp = edge(im_tmp, detection_type);
end
